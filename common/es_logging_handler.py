...
class ESLoggingHandler(logging.Handler):
    ...
    def __init__(self,
        ...
        :param auth_details: When ```ESLoggingHandler.AuthType.BASIC_AUTH``` is used this argument must contain
        ...
        :param auth_type: The authentication type to be used in the connection ```ESLoggingHandler.AuthType```
        ...
        :param raise_on_indexing_exceptions: A boolean, True only for debugging purposes to raise exceptions
                    caused when
        :return: A ready to be used ESLoggingHandler.
        """
    ...
    def __get_es_client(self):
        if self.auth_type == ESLoggingHandler.AuthType.NO_AUTH:
            return Elasticsearch(hosts=self.hosts,
                                 use_ssl=self.use_ssl,
                                 verify_certs=self.verify_certs,
                                 connection_class=RequestsHttpConnection)
        elif self.auth_type == ESLoggingHandler.AuthType.BASIC_AUTH:
            return Elasticsearch(hosts=self.hosts,
    ...
    def emit(self, record):
        ...
        for k, v in record.__dict__.items():
            if k not in ESLoggingHandler.__LOGGING_FILTER_FIELDS:
